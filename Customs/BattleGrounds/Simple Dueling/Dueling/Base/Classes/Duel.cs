...
namespace Server.Dueling // Declare the classes to be in the Server.Dueling namespace
{
    public class Duel // Declare the public class duel
    {
        ...
        public Duel_StartTimer StartTimer;
        ...
        public Duel_Add_Timer AddTimer; // Declares AddTimer, so that the AddTimer cannot have multiple instances per duel
        ...
        public bool Bless = true, Fireball = true, MagicLock = true, Poison = true, Telekinisis = true, Teleport = false, Unlock = true, WallOfStone = false;
        public bool ArchCure = true, ArchProtection = true, Curse = true, FireField = false, GreaterHeal = true, Lightning = true, ManaDrain = true, Recall = false;
        public bool BladeSpirits = false, DispelField = true, Incognito = false, MagicReflection = false, MindBlast = true, Paralyze = false, PoisonField = false, SummonCreature = false;
        public bool Dispel = true, EnergyBolt = true, Explosion = true, Invisibility = false, Mark = false, MassCurse = true, ParalyzeField = false, Reveal = true;
        public bool ChainLightning = true, EnergyField = false, FlameStrike = true, GateTravel = false, ManaVampire = true, MassDispel = true, MeteorSwarm = true, Polymorph = true;
        public bool EarthQuake = false, EnergyVotex = false, Resurrection = false, SummonAirElemental = false, SummonDaemon = false, SummonEarthElemental = false, SummonFireElemental = false, SummonWaterElemental = false;
        ...
        public bool Anatomy = true, DetectHidden = true, EvaluatingIntelligence = true, Hiding = false, Poisoning = false, Snooping = false, Stealing = false, SpiritSpeak = false, Stealth = false;
        ...
        public bool Bandages = true, TrappedContainers = true, Bolas = false, Mounts = false, Wands = false;
    ...
    public class Duel_StartTimer : Timer
    {
        public Duel Handeling;
        public int C, A;

        public Duel_StartTimer(Duel d, int count, int attempts)
        ...
        protected override void OnTick()
        {
            ...
            if (A >= 4)
            {
                ...
                Handeling.EchoMessage("Duel starting timed out, please wait.");
                this.Stop();
                return;
            }

            if (C >= Duel_Config.DuelStartDelay)
            {
                C = 0;
                AttemptStart();
            }
            ...
            {
                this.Stop();
                Duel_StartTimer tmr = new Duel_StartTimer(Handeling, (C + 1), A);
            }
        }
	...
	public DuelController PickRandomArena()
	{
        ...
        public DuelController ArenaAvaiable()
        {
	    foreach (DuelController c in Duel_Config.Controllers)
            {
