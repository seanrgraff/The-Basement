...
{
	public class RVSSetup_ParticipantSetup : Gump
	{
        public RVS Handeling;
        public RVS_Team D_Team;
        public int TeamID;
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
            ...
            switch (info.ButtonID)
            {
                case 0:
                    {
                        from.CloseGump(typeof(RVSSetup_Main));
                ...
                case 1:
                    {
                        if (D_Team.Size >= 6)
                        {
                            ...
                            from.SendGump(new RVSSetup_ParticipantSetup(Handeling, TeamID));
                            return;
                        }
            ...
            if (info.ButtonID >= 3)
            {
                ...
                {
                    from.Target = new RVS_AddTarget(Handeling, TeamID, index);
                    return;
                }
            }
		}
    ...
    public class RVSSetup_AddParticipant : Gump
    {
        public RVS Handeling;
        public int TeamID;
        public RVS_Team D_Team;
    ...
    public class RVS_AddTarget : Target
    {
        public RVS Handeling;
        public int TeamID;
        public RVS_Team D_Team;
        ...
        public bool CanTarget(Mobile from, object o)
        {
            ...
            if (RVS_Config.InARVS(target) || Duel_Config.InADuel(target))
            {
                pm.SendMessage("They are already in a RVS duel.");
                return false;
            }

            if (!RVS_Config.AllowSameIPRVSs && SameIP(pm, (PlayerMobile)target))
            {
        ...
        protected override void OnTarget(Mobile from, object targeted)
        {
            if (CanTarget(from, targeted))
            {
                ...
                RVS_Add_Timer tmr = new RVS_Add_Timer((PlayerMobile)from, tosend, Handeling, TeamID);
                Handeling.AddTimer = tmr;
                tosend.SendGump(new RVSSetup_AddParticipant(Handeling, TeamID, Index));
            }
            else
                from.SendGump(new RVSSetup_ParticipantSetup(Handeling, TeamID));
        }
    ...
    public class RVS_Add_Timer : Timer
    {
        PlayerMobile Sent;
        ...
        {
            if (Recieved.HasGump(typeof(RVSSetup_AddParticipant)))
            {
                Sent.SendMessage(String.Format("{0} seems to be unresponsive.", Recieved.Name));
                Recieved.CloseGump(typeof(RVSSetup_AddParticipant));
                Sent.SendGump(new RVSSetup_ParticipantSetup(Handeling, TeamID));
            }
