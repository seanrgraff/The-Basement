...
{ 
	public class FriendlyPlayer : BaseBlue
	{ 
...
//       		 public override bool CanHeal { get { return true; } }
		...
		{ 
			private FriendlyPlayer pk;

			public BandageTimer( FriendlyPlayer o ) : base( TimeSpan.FromSeconds( 4 ) ) 
			{ 
				pk = o;
		...
		public override void OnMovement( Mobile m, Point3D oldLocation )
		{
			if ( InRange( m, 4 ) && !InRange( oldLocation, 4 ) && InLOS( m ) )
			{
				...
				else if ( DateTime.Now >= m_NextResurrect && m.Hits < (m.HitsMax/2) && m is PlayerMobile || DateTime.Now >= m_NextResurrect && m.Hits < (m.HitsMax/2) && m is BaseBlue )
				{
		...
		public FriendlyPlayer( Serial serial ) : base( serial ) 
		{ 
		} 
